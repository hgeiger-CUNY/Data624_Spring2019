...
Let's look at what variables are included in a linear model.
...
Get the top 10 most important predictors.

```{r}
coefficients <- as.numeric(as.vector(coef(linear_model)))
coefficients  <- coefficients[2:length(coefficients)]

coefficients <- data.frame(Variable = names(coef(linear_model))[2:length(coef(linear_model))],
                    Coef.absv = abs(coefficients),
                    Direction = ifelse(coefficients >= 0,"+","-"))

coefficients[order(coefficients$Coef.absv,decreasing=TRUE)[1:10],]
```

It appears that manufacturing processes have the most influence in the linear model.
