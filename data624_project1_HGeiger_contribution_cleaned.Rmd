...
For the one obvious S02-Var03 outlier, get the value that would come from a z-score equal to 1/n, where n is the number of non-NA values we currently have.

Use this to fill in and make the outlier less extreme.

```{r}
S02_Var03 <- data_gathered$Value[data_gathered$Group.plus.var == "S02-Var03"]
S02_Var03_max <- max(S02_Var03,na.rm=TRUE)
S02_Var03_minus_max <- S02_Var03[which(is.na(S02_Var03) == FALSE & S02_Var03 < S02_Var03_max)]

non_NA_values <- length(S02_Var03_minus_max) + 1
max_zscore <- -1*qnorm(1/non_NA_values)
new_value <- mean(S02_Var03_minus_max) + max_zscore*sd(S02_Var03_minus_max)
...
Let's use a very simple strategy to fill in NAs for non-consecutive missing dates.

Average the nearest non-missing day before and day after.
...
```{r}
missing_dates_non_consecutive <- setdiff(missing_dates,
	as.Date(c("2014-02-28","2014-03-01"),origin="1970-01-01"))
...
for(var in variables)
{
	...
	for(date in as.Date(missing_dates_non_consecutive,origin="1970-01-01"))
	{
	...
	data_gathered <- rbind(data_gathered,
		data.frame(SeriesInd = as.Date(missing_dates_non_consecutive,origin="1970-01-01"),
...
```{r}
data_gathered <- data_gathered[order(data_gathered$SeriesInd),]

dates_in_data <- unique(data_gathered$SeriesInd)
```

For consecutive missing dates, use a slightly modified strategy (weighted average).

1. First missing day - 2/3 nearest day before + 1/3 nearest day after
2. Second missing day - 1/3 nearest day before + 2/3 nearest day after

```{r}
data_gathered <- rbind(data_gathered,
	data.frame(SeriesInd = as.Date(rep(c("2014-02-28","2014-03-01"),times=6),origin="1970-01-01"),
		group = rep(unlist(lapply(strsplit(variables,"-"),"[[",1)),each=2),
		Variable = rep(unlist(lapply(strsplit(variables,"-"),"[[",2)),each=2),
		Value = NA,
		Group.plus.var = rep(variables,each=2),
		stringsAsFactors=FALSE))
```
...
for(var in variables)
{
	...
	present_dates <- data_this_var$SeriesInd[which(is.na(data_this_var$Value) == FALSE)]
	absent_dates <- data_this_var$SeriesInd[which(is.na(data_this_var$Value) == TRUE)]

	to_fill_in <- rep(NA,times=length(absent_dates))

	for(i in 1:length(absent_dates))
	{
		date = as.Date(absent_dates[i],origin="1970-01-01")

		this_nearest_before <- present_dates[present_dates < date]
		this_nearest_after <- present_dates[present_dates > date]

		this_nearest_before <- this_nearest_before[length(this_nearest_before)]
		this_nearest_after <- this_nearest_after[1]

		if((i %% 2) == 1){to_fill_in[i] <- mean(c(this_nearest_before,this_nearest_before,this_nearest_after))}
		if((i %% 2) == 0){to_fill_in[i] <- mean(c(this_nearest_before,this_nearest_after,this_nearest_after))}
	}

	data_gathered <- rbind(data_gathered,
		data.frame(SeriesInd = as.Date(absent_dates,origin="1970-01-01"),
            group = unique(data_this_var$group),
            Variable = unique(data_this_var$Variable),
            Value = to_fill_in,
            Group.plus.var = var,
...
```{r}
data_gathered <- data_gathered[which(is.na(data_gathered$Value) == FALSE),]

data_gathered <- data_gathered[order(data_gathered$SeriesInd,data_gathered$Group.plus.var),]
```

```{r,echo=FALSE,eval=FALSE}
new_dates <- unique(data_gathered$SeriesInd)

table(weekdays(new_dates))

all_dates <- as.Date(seq(from = min(data_gathered$SeriesInd),to = max(data_gathered$SeriesInd),by="days"),origin="1970-01-01")

all_dates <- all_dates[weekdays(all_dates) != "Wednesday" & weekdays(all_dates) != "Thursday"]

as.Date(setdiff(all_dates,new_dates),origin="1970-01-01")
```

### Filling in Wednesdays and Thursdays

Now we just need to fill in Wednesdays and Thursdays.
